...
<head>
    ...
    <style>
        .instructor-seleccionado {
            background-color: rgb(168, 0, 0); /* Color de selección */
        }
        tr {
            cursor: pointer;
        }
...
<body class="bg-gray-100">
    ...
    <script>
        ...
        function seleccionarInstructor(id) {
            // Si se hace clic en el mismo instructor, deseleccionarlo
            if (instructorSeleccionadoId === id) {
                ...
                // Deseleccionar cualquier otro instructor previamente seleccionado
                document.querySelectorAll('.tabla-ficha tr').forEach(row => {
                    row.classList.remove('instructor-seleccionado');
                });

                // Seleccionar el nuevo instructor
                instructorSeleccionadoId = id;
                document.getElementById('instructor-' + id).classList.add('instructor-seleccionado');
                document.getElementById('btn-inactivar').disabled = false;  // Habilitar el botón de inactivación
            }
        }

        // Función para inactivar el instructor
        function inactivarInstructor() {
            if (!instructorSeleccionadoId) return;

            // Realizar la solicitud para inactivar el instructor en el backend
            ...
            .then(response => {
                if (response === 'OK') {
                    // Eliminar la fila de la tabla para reflejar la inactivación
                    document.getElementById('instructor-' + instructorSeleccionadoId).remove();
                    instructorSeleccionadoId = null;
                    document.getElementById('btn-inactivar').disabled = true;  // Deshabilitar el botón
                } else {
